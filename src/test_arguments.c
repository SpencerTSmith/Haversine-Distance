...
int main(int argc, char **argv)
{
  ...
  for (isize i = 0; i < arguments.raw_strings_count; i++)
  {
    printf("Raw %td: %.*s\n", i, String_Format(arguments.raw_strings[i]));
  }
  ...
  for (isize i = 0; i < arg->values_count; i++)
  {
    printf("Value of %.*s: %.*s\n", String_Format(arg->name), String_Format(arg->values[i]));
  }
